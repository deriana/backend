...
const insertAdmin = async (req, res) => {
    ...
    try {
        ...
        const results = await adminModel.insertAdmin(username, hashedPassword, email);
...
const editAdmin = async (req, res) => {
    ...
    if (!id || !username || !email) {
        return res.status(400).json({ message: "ID, username, dan email wajib diisi" });
    }
    ...
    try {
        ...
        const results = await adminModel.editAdmin(id, username, email, hashedPassword);
